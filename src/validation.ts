 ...
 * Returns an array of validation errors/warnings. An empty array means the document is valid.
 */
export function validate(doc: AnmlDocument): ValidationError[] {
  ...
  if (!doc.version) {
    ...
    errors.push({
      path: "version",
      message: `Unsupported version "${doc.version}". Only "1.0" is supported.`,
      severity: "warning",
    });
  }
  ...
  if (doc.head) {
    if (doc.head.meta) {
      for (let i = 0; i < doc.head.meta.length; i++) {
        ...
        if (!meta.name) {
          errors.push({
            path: `head.meta[${i}].name`,
            message: "Meta entry must have a name",
            severity: "error",
          });
        }
        if (!meta.value) {
          errors.push({
            path: `head.meta[${i}].value`,
            message: "Meta entry must have a value",
            severity: "error",
          });
        }
      }
  ...
  if (doc.constraints?.disclosure) {
    ...
    for (let i = 0; i < doc.constraints.disclosure.length; i++) {
      ...
      if (!d.field) {
        errors.push({
          path: `constraints.disclosure[${i}].field`,
          message: "Disclosure must reference a field",
          severity: "error",
        });
      }
      if (!d.requires) {
        errors.push({
          path: `constraints.disclosure[${i}].requires`,
          message: "Disclosure must specify a requires value",
          severity: "error",
        });
      } else if (!validRequires.includes(d.requires)) {
        errors.push({
          path: `constraints.disclosure[${i}].requires`,
          message: `Invalid requires value "${d.requires}". Must be one of: ${validRequires.join(", ")}`,
          severity: "error",
        });
      }
    }
  ...
  if (doc.state) {
    ...
    // Context step must reference a valid flow step
    if (doc.state.context?.step && flowStepIds.size > 0) {
      if (!flowStepIds.has(doc.state.context.step)) {
        errors.push({
          path: "state.context.step",
          message: `Context step "${doc.state.context.step}" does not match any flow step id`,
          severity: "error",
        });
      }
    }
  ...
  if (doc.interact?.action) {
    ...
    for (let i = 0; i < doc.interact.action.length; i++) {
      ...
      if (!action.endpoint) {
        errors.push({
          path: `interact.action[${i}].endpoint`,
          message: "Action must have an endpoint",
          severity: "error",
        });
      }
      ...
      if (action.method) {
        ...
        if (!validMethods.includes(action.method)) {
          errors.push({
            path: `interact.action[${i}].method`,
            message: `Invalid HTTP method "${action.method}". Must be one of: ${validMethods.join(", ")}`,
            severity: "error",
          });
        }
      }
      ...
      if (action.params) {
        ...
        for (let j = 0; j < action.params.length; j++) {
          ...
          if (!param.name) {
            errors.push({
              path: `interact.action[${i}].params[${j}].name`,
              message: "Param must have a name",
              severity: "error",
            });
          } else {
            if (paramNames.has(param.name)) {
              errors.push({
                path: `interact.action[${i}].params[${j}].name`,
                message: `Duplicate param name "${param.name}" in action "${action.id}"`,
                severity: "warning",
              });
            }
    ...
    if (doc.knowledge?.ask) {
      for (let i = 0; i < doc.knowledge.ask.length; i++) {
        ...
        if (ask.action && actionIds.size > 0 && !actionIds.has(ask.action)) {
          errors.push({
            path: `knowledge.ask[${i}].action`,
            message: `Ask references action "${ask.action}" which is not defined in interact section`,
            severity: "error",
          });
        }
      }
  ...
  if (doc.knowledge) {
    if (doc.knowledge.ask) {
      for (let i = 0; i < doc.knowledge.ask.length; i++) {
        ...
        if (!ask.field) {
          errors.push({
            path: `knowledge.ask[${i}].field`,
            message: "Ask must have a field",
            severity: "error",
          });
        }
        if (!ask.action) {
          errors.push({
            path: `knowledge.ask[${i}].action`,
            message: "Ask must reference an action",
            severity: "error",
          });
        }
      }
    ...
    if (doc.knowledge.inform) {
      for (let i = 0; i < doc.knowledge.inform.length; i++) {
        ...
        if (!inform.content) {
          errors.push({
            path: `knowledge.inform[${i}].content`,
            message: "Inform must have content",
            severity: "error",
          });
        }
      }
  ...
  if (doc.footer?.rights?.usage) {
    ...
    if (!validUsage.includes(doc.footer.rights.usage)) {
      errors.push({
        path: "footer.rights.usage",
        message: `Invalid usage value "${doc.footer.rights.usage}". Must be one of: ${validUsage.join(", ")}`,
        severity: "error",
      });
    }
  }
